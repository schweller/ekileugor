...
use crate::{game::GameLog, components::{PickupItemIntent, Position, Name, ItemOwned, Item, ActiveEntity}};
...
impl<'a> System<'a> for InventorySystem {
    ...
    fn run(&mut self, data: Self::SystemData) {
        let (active_entity, entities, mut log, mut pickup_intent, mut pos, names, mut item_owned) = data;

        for pickup in pickup_intent.join() {
            pos.remove(pickup.item);
            item_owned.insert(pickup.picked_by, ItemOwned{ owner: pickup.picked_by }).expect("Unable to insert ItemOwned");
