...
use crate::{map::{Map, TileType}, components::{ActiveEntity, Position, Renderable}};
...
pub fn render_camera(ecs: &World, ctx : &mut BTerm) {
    ...
    let player_pos = positions.get(active_entity.target).unwrap();
    // let (x_chars, y_chars) = ctx.get_char_size();
    let (x_chars, y_chars) = (16,16);
